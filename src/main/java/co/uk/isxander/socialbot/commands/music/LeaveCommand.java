...
public class LeaveCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (member.hasPermission(Permission.ADMINISTRATOR) || member.getRoles().contains(musicRole)) {
            ...
            AudioManager audioManager = guildInstance.getAudioManager();
            if (!audioManager.isConnected()) {
                channel.sendMessage(Embeds.error("I am not connected to a voice channel!")).queue();
                return;
            }
            ...
            channel.sendMessage(Embeds.success("Disconnected from the voice channel, goodbye!")).queue();
        }
        else {
